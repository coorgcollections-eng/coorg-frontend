...
//         mode: "production"  // "production", // mode: "sandbox"   
//       });
...
//       } catch (sdkErr) {
//         console.log("Cashfree Checkout Failed:", sdkErr);
//         alert("Cashfree Checkout Failed - check console/network response");
//       }
...
export default function Checkout() {
  ...
  const handlePayNow = async () => {
    try {
      ...
      // 2️⃣ Create Cashfree Payment Session
      const sessionRes = await API.post(
        `/order/cashfree/session/${orderId}`
      );

      console.log("Session Response:", sessionRes.data);

      const paymentSessionId =
        sessionRes.data.data.paymentSessionId;
